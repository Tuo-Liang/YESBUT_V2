...
def resize_image_to_1024(image_path):
    """
    将图片的宽或高调整到 1024 像素以内，同时保持宽高比。
    ...
    with Image.open(image_path) as img:
        ...
        if max(original_width, original_height) > 1024:
            ...
            scaling_factor = 1024 / max(original_width, original_height)
...
def main():
    ...
    for sample in data:
        ...
        img1 = resize_image_to_1024(image_path1)
        img2 = resize_image_to_1024(image_path2)
        img = [img1,img2]
...
#     parser.add_argument('--gen_con', action= "store_true")
#     parser.add_argument('--icot', action= "store_true")
#     parser.add_argument('--prompt_num', type=int, required=True, default=0)
#     args = parser.parse_args()
#     print(args)
...
#             #get prompt
#             #prompt=get_prompt("Literally describe these two panels respectively.")
#             prompt=get_prompt(gen_description(num= args.prompt_num))
...
#         if args.gen_con and args.icot!=True:
#             print("---------generate contradiction--------")
#             prompt=get_prompt(formulate_instruction_contradiction_generation(caption= None, num= args.prompt_num))
...
#         #only img input
#         if args.gen_des!=True and args.gen_con!=True:
#             instruction = formulate_instruction(sample, caption= None, task=args.task,num = args.prompt_num)
        
#         if args.gen_des!=True and args.gen_con:
#             instruction = formulate_instruction(sample, caption= contradiction, task=args.task,num = args.prompt_num)

#         if args.gen_des and args.gen_con!=True:
#             instruction = formulate_instruction(sample, caption= cur_caption, task=args.task,num = args.prompt_num)
